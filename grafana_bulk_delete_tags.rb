#!/usr/bin/env ruby
# Delete dashboards from grafana in bulk based on tags
# Usage: grafana_bulk_delete_tags.rb http://grafana.example.com/ TAG
...
def search_dashboards(grafana_url, query)
  ...
  uri = URI.parse("#{grafana_url}/api/search?limit=0&tag=#{query}")
...
if ARGV[1].nil?
  puts "Usage: #{$0} URL TAG"
  puts "Bulk delete dashboards based on tag"
end
